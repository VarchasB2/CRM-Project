...
const ContactLookup = async () => {
  ...
  return (
    <div className="grid flex-1 items-start gap-4 p-4 sm:px-6  md:gap-8 ">
      <ContactLookupTable data={data} columns={columns} firstDate={data.length===0?today:data[0].lead[0].date} lastDate={data.length===0?today:data[data.length-1].lead[data[data.length-1].lead.length-1].date}/>
    </div>
  )
}
